...
- 通过 `Mz = -气动拖距 × Fy + 残余力矩` 定义并绘制多载荷回正力矩
- 绘制气动拖距随侧偏角和载荷变化的曲线
...
Mz 设计参数会保存在 `tyre_design.json` 并作为预览显示。工具不会通过 Lua 向轮毂注入附加扭矩，因为这种方法会干扰悬架约束并导致车辆物理不稳定。AC/CSP 当前没有公开的 `tyres.ini` 字段可直接输入任意 Mz 曲线。

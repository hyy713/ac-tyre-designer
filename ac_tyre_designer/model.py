...
@dataclass
class AligningMomentParams:
    """Semi-empirical aligning moment model: Mz = -trail*Fy + Mzr."""

    trail0_m: float = 0.075
    trail_decay: float = 7.0
    trail_load_exp: float = -0.10
    residual_mu0: float = 0.015
    residual_B: float = 7.0
    residual_C: float = 1.0
    residual_E: float = 0.0
    residual_load_exp: float = -0.05
...
class TyreDefinition:
    ...
    aligning: AligningMomentParams = None  # type: ignore[assignment]
    ...
    def __post_init__(self) -> None:
        ...
        if self.aligning is None:
            self.aligning = AligningMomentParams()
...
def pneumatic_trail(slip_angle_rad: np.ndarray, load_n: float, ref_load_n: float,
                    p: AligningMomentParams) -> np.ndarray:
    ratio = max(load_n / ref_load_n, 1e-6)
    return p.trail0_m * ratio ** p.trail_load_exp * np.exp(-p.trail_decay * np.abs(slip_angle_rad))


def aligning_moment(slip_angle_rad: np.ndarray, load_n: float, tyre: TyreDefinition) -> np.ndarray:
    """Return self-aligning torque in N m using Mz = -t*Fy + Mzr."""
    alpha = np.asarray(slip_angle_rad, dtype=float)
    fy = magic_formula(alpha, load_n, tyre.reference_load_n, tyre.lateral)
    trail = pneumatic_trail(alpha, load_n, tyre.reference_load_n, tyre.aligning)
    p = tyre.aligning
    ratio = max(load_n / tyre.reference_load_n, 1e-6)
    bx = p.residual_B * alpha
    residual_shape = np.sin(p.residual_C * np.arctan(bx - p.residual_E * (bx - np.arctan(bx))))
    residual = load_n * tyre.radius_m * p.residual_mu0 * ratio ** p.residual_load_exp * residual_shape
    return -trail * fy + residual
...
def load_definition(path: Path) -> TyreDefinition:
    ...
    payload["aligning"] = AligningMomentParams(**payload.get("aligning", {}))
...
def load_unitire_definition(path: Path) -> TyreDefinition:
    ...
    mz_data = payload.get("mz", {})
    residual_lever = float(mz_data.get("residualAlpha_m", 0.0))
    residual_b = 1.0
    residual_c = 1.0
    residual_mu = residual_lever / max(radius * residual_b * residual_c, 1e-6)
    aligning = AligningMomentParams(
        trail0_m=float(mz_data.get("trail0_m", 0.055)),
        trail_decay=float(mz_data.get("trailDecay", 4.5)),
        trail_load_exp=0.0,
        residual_mu0=float(np.clip(residual_mu, -1.0, 1.0)),
        residual_B=residual_b,
        residual_C=residual_c,
        residual_E=0.0,
        residual_load_exp=0.0,
    )
...
; Target Mz model is a design preview only; AC/CSP has no direct curve input:
; MZ_TRAIL0_M={mz.trail0_m:.7f}, MZ_TRAIL_DECAY={mz.trail_decay:.7f}, MZ_TRAIL_LOAD_EXP={mz.trail_load_exp:.7f}
; MZ_RESIDUAL_MU0={mz.residual_mu0:.7f}, MZ_RESIDUAL_B={mz.residual_B:.7f}, MZ_RESIDUAL_C={mz.residual_C:.7f}, MZ_RESIDUAL_E={mz.residual_E:.7f}
